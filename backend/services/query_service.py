...
class SessionContext:
    ...
    def __init__(self, session_id: str):
        ...
        # Keep the exact verification snapshot used for the previous weather answer.
        # This prevents a follow-up like "why this confidence?" from triggering a
        # second live provider comparison and changing MEDIUM to HIGH/LOW.
...
class QueryService:
    ...
    def parse_query(cls, raw_message: str, current_location: Optional[Location], session_id: Optional[str]) -> ParsedQuery:
        ...
        # Normalize punctuation/hyphens so natural variations like "7-day", "7 day",
        # "7-day forecast?" and "7 day forecast" are understood the same way.
        lower = re.sub(r"[-–—]+", " ", clean.lower())
        ...
        # Weekly forecast requests are their own time range. Do not let a
        # previous message's "tomorrow"/weekday context leak into them.
    ...
    def _classify_intent(
        ...
        if any(w in lower for w in [
            ...
            "verify", "verification", "verified", "trust", "compare sources",
            "compare providers", "other source", "other sources", "other site",
            "other sites", "source agreement", "how sure", "how certain",
            "can i trust", "why trust", "why is this accurate", "why is the forecast accurate",
            "why is this forecast accurate", "why is the forecast high", "why high confidence",
            "why high accuracy", "is this accurate", "is this reliable",
            "is this forecast reliable", "proof of the confidence", "proof of confidence"
        ]):
        ...
        # Be tolerant of hyphens, spacing, wording and common conversational forms.
        weekly_patterns = [
            r"\b7\s*day(?:s)?\b",
            r"\bseven\s*day(?:s)?\b",
            r"\bnext\s+(?:7|seven)\s+day(?:s)?\b",
            r"\b(?:next|coming|upcoming)\s+week\b",
            r"\bthis\s+week(?:'s)?\b",
            r"\bweek(?:ly)?\s+forecast\b",
            r"\bforecast\s+(?:for|over)\s+(?:the\s+)?(?:next|coming|upcoming)\s+week\b",
            r"\bforecast\s+(?:for|over)\s+(?:the\s+)?(?:next|coming)\s+(?:7|seven)\s+day(?:s)?\b",
            r"\bweather\s+(?:for|over)\s+(?:the\s+)?(?:next|coming|upcoming)\s+week\b",
            r"\bweather\s+(?:for|over)\s+(?:the\s+)?(?:next|coming)\s+(?:7|seven)\s+day(?:s)?\b",
        ]
        if any(re.search(pattern, lower) for pattern in weekly_patterns):
        ...
        # 13. General Forecast
        forecast_phrases = [
            "forecast", "weather outlook", "weather prediction", "what will the weather be like",
            "what's the weather going to be like", "how will the weather be",
            "how is the weather expected to be", "what should i expect",
            "what can i expect from the weather"
        ]
        if any(p in lower for p in forecast_phrases):
            return "forecast_7day" if any(re.search(pattern, lower) for pattern in weekly_patterns) else "current_weather"
    ...
    def _extract_city_name(cls, text: str) -> Optional[str]:
        ...
        for pattern in patterns:
            ...
            # Do not mistake conversational phrases for city names. This is
            # especially important for queries such as "7-day forecast",
            # "how accurate is this", or "can I trust this forecast".
            non_city_tokens = {
                "7", "7day", "7-day", "days", "day", "seven", "week", "weekly",
                "next", "coming", "upcoming", "forecast", "outlook",
                "accuracy", "accurate", "confidence", "confident",
                "reliable", "reliability", "verify", "verification", "verified",
                "trust", "proof", "sure", "certain", "chance", "any", "of",
                "there", "should", "could", "would", "can", "i", "this", "that",
                "is", "are", "will", "it", "going", "to", "rain", "weather"
            }
            normalized_tokens = {token.lower().replace("-", "") for token in tokens}
            normalized_non_city = {token.replace("-", "") for token in non_city_tokens}
            if tokens and not any(token in normalized_non_city for token in normalized_tokens):
                return " ".join(tokens)
        ...
        action_markers = [
            ...
            "accuracy", "accurate", "confidence", "confident", "reliable", "reliability",
            "verify", "verification", "verified", "trust", "compare sources", "compare providers",
            "source agreement", "proof of confidence", "forecast", "weather outlook",
            "7 day", "7-day", "7day", "seven day", "weekly forecast", "next week", "this week",
            "coming week", "upcoming week", "chance of rain", "any chance",
        ...
        if stripped and len(stripped.split()) <= 6:
            ...
            generic_tokens = {
                "7", "day", "days", "seven", "week", "weekly", "forecast", "outlook",
                "weather", "today", "tomorrow", "next", "coming", "upcoming",
                "accurate", "accuracy", "confidence", "confident", "reliable", "reliability",
                "verify", "verification", "verified", "trust", "proof", "sure", "certain"
            }
            if tokens and not all(t.lower() in generic_tokens for t in tokens):
                return " ".join(tokens)
